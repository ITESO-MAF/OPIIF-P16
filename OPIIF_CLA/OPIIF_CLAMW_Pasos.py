...
# -- Desarrollador Inicial: LopezDePrado ------------------------------------------- -- #
# -- Licencia: MIT ----------------------------------------------------------------- -- #
# -- ------------------------------------------------------------------------------- -- #
...
# -- Descargar archivo.txt de la pagina de internet ------------------ Datos Ejemplo -- #
...
Simbolo = stock_dict.keys()
Tickers = stock_dict.values()

Fecha_Fin = datetime.today() - timedelta(days=150)
Fecha_Ini = Fecha_Fin - timedelta(days=10)

Precios = download_data(Simbolo[1], Fecha_Fin, Fecha_Ini)
PreciosCl = ([Precios.TimeStamp, Precios.Close])

stocks_data = pd.DataFrame(Simbolo, columns=["Ticker"])
stocks_data["Empresa"] = Tickers

mean1 = np.array(data[:1]).T
lB1 = np.array(data[1:2]).T
uB1 = np.array(data[2:3]).T
cov1 = np.array(data[3:])
